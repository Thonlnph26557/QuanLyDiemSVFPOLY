...
namespace QLDSVFPOLY.Blazor.Repository.Implements
{
    public class ChiTietDiemSoRepo : IChiTietDiemSoRepo
    {
        ...
        public async Task<ChiTietDiemSoVM> GetByIdAsync(Guid id)
        {
            var result = await _httpClient.GetFromJsonAsync<ChiTietDiemSoVM>($"/api/ChiTietDiemSos/all?Id={id}");
            return result;
        }

        public async Task<int> CreateAsync(ChiTietDiemSoCreateVM vm)
        {
            var result = await _httpClient.PostAsJsonAsync("api/ChiTietDiemSos", vm);
            if (result.IsSuccessStatusCode) return 1;
            return 0;
        }
        public async Task<int> UpdateAsync(Guid id, ChiTietDiemSoUpdateVM vm)
        {
